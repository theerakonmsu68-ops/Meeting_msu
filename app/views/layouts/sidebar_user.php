...
// sidebar_user.php อยู่ที่ app/views/layouts/
// ==========================================
if (
    !defined('BASE_URL') ||
    !defined('APP_PATH') ||
...
$sidebar_public_base = rtrim((string)PUBLIC_URL, '/') . '/';
    ...
    if (filter_var($sidebar_picture, FILTER_VALIDATE_URL)) {
        ...
        if ($picture_file !== '' && $picture_file !== '.' && $picture_file !== '..') {
            $sidebar_avatar = $sidebar_public_base
                ...
                . '?v=' . rawurlencode((string) @filemtime(PUBLIC_PATH . '/uploads/avatars/' . $picture_file));
        }
    }
}
...
<div class="sidebar-wrapper<?= $sidebar_collapsed ? ' collapsed' : '' ?>" id="sidebar">
    ...
    <nav class="sidebar-menu">
        <a href="<?= PUBLIC_URL ?>users/index.php" class="menu-item <?= $current_page == 'dashboard' ? 'active' : '' ?>">
            <i data-lucide="layout-dashboard"></i>
            <span class="menu-text">หน้าแรก / การประชุม</span>
        </a>

        <a href="<?= PUBLIC_URL ?>users/calendar.php" class="menu-item <?= $current_page == 'calendar' ? 'active' : '' ?>">
            <i data-lucide="calendar-days"></i>
            <span class="menu-text">ปฏิทินการประชุม</span>
        </a>

        <a href="<?= PUBLIC_URL ?>users/meeting_history.php" class="menu-item <?= $current_page == 'history' ? 'active' : '' ?>">
    ...
    <div class="sidebar-footer">
        <a href="<?= PUBLIC_URL ?>auth/logout.php" class="btn-logout"
...
<script>
    ...
    window.toggleUserSidebar = function () {
        ...
        const cookiePath = <?= json_encode(
            BASE_URL,
            JSON_UNESCAPED_SLASHES
        ) ?>;

        document.cookie =
            'meeting_sidebar_collapsed=' +
            (isCollapsed ? '1' : '0') +
            '; path=' + cookiePath +
