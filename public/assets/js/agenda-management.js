...
async function viewAgenda(id) {
    ...
    try {
        const response = await fetch(`${window.APP_URL}controllers/Agenda_api.php?action=detail&agenda_id=${id}`);
...
function renderAgendaDetail(agenda) {
    ...
    if (agenda.documents && agenda.documents.length > 0) {
        ...
        ${
            ...
            <a class="document-link" href="${window.APP_URL}controllers/download.php?file=${encodeURIComponent(doc.file_path)}" target="_blank">
...
async function postAgendaAction(action, id, message, confirmTitle = 'ยืนยันการทำรายการ', isDangerous = false) {
    ...
    try {
        ...
        const response = await fetch(`${window.APP_URL}controllers/Agenda_api.php?action=${action}`, {
            method: 'POST',
            body: formData
        });
