    ...
    const existingDocuments = Array.isArray(documents)
        ? documents.map(doc => `
            <div class="agenda-existing-doc" data-document-id="${Number(doc.agenda_document_id)}">
                <a href="${window.APP_URL}controllers/download.php?file=${encodeURIComponent(doc.file_path)}" target="_blank" title="${escapeAttribute(doc.document_name)}">
                    ...
                    ? `
                        <div class="agenda-card-documents">
                            ...
                            ${agenda.documents.map(doc => `
                                <a href="${window.APP_URL}controllers/download.php?file=${encodeURIComponent(doc.file_path)}" target="_blank">
                                    📄 ${escapeHtml(doc.document_name)}
                                </a>
                            `).join('')}
                        </div>
